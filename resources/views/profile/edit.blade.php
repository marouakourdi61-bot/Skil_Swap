                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                        ...
                        <div class="bg-[#fffaf7] p-6 rounded-2xl border border-[#f5e5d8] space-y-4">
                            ...
                            <input type="text" name="name" value="{{ old('name', auth()->user()->name) }}"
                                class="w-full border rounded-lg p-2" placeholder="Name">

                            <input type="email" name="email" value="{{ old('email', auth()->user()->email) }}"
                                class="w-full border rounded-lg p-2" placeholder="Email">
                            ...
                            <input type="text" name="location" value="{{ old('location', auth()->user()->location) }}"
                                class="w-full border rounded-lg p-2" placeholder="Location">

                            <textarea name="bio" rows="4" class="w-full border rounded-lg p-2"
                                placeholder="Your bio...">{{ old('bio', auth()->user()->bio) }}</textarea>

                            <textarea name="availability" rows="3" class="w-full border rounded-lg p-2"
